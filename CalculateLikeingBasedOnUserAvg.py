...
def convertUserDataDependingOnUserAvg():
    ...
    with open('userAvgRating.dat', 'r') as r:
        with open('ratings.csv', 'r') as f:
            with open('userMlAboveAvg.dat', 'w+') as wP:
                with open('userMlBelowAvg.dat', 'w+') as wN:
                    # read file into a csv reader
                    reader = csv.reader(f, delimiter=",")
                    tmp = list(reader)
                    # Create and array or enough size for max(user_id) around 950
                    out_pos = [[-1 for i in range(1)] for j in range(138495)]
                    out_neg = [[-1 for i in range(1)] for j in range(138495)]
                    ...
                    rating = [-1 for j in range(138495)]
